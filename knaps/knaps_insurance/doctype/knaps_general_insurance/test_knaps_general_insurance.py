...
class TestKNAPSGeneralInsurance(IntegrationTestCase):
	def setUp(self):
		super().setUp()
		frappe.db.savepoint("knaps_general_insurance_sp")
		...
		self.insurance_type = self._create_insurance_type("Fire")

	def tearDown(self) -> None:
		frappe.db.rollback(save_point="knaps_general_insurance_sp")
		super().tearDown()
	...
	def test_multi_member_primary_from_holders(self):
		doc = self._make_policy(has_multiple_members=1, primary_client=None)
		self._add_member(doc, self.adult_client, is_primary=True)
		self._add_member(doc, self.other_client)
		self._add_nominee(doc, self.nominee_individual)
		self._add_payment(doc)
		doc.insert()
	...
	def test_multi_member_no_primary_throws(self):
		doc = self._make_policy(has_multiple_members=1, primary_client=None)
		self._add_member(doc, self.adult_client)
		self._add_member(doc, self.other_client)
		self._add_nominee(doc, self.nominee_individual)
		self._add_payment(doc)
	...
	def test_multi_member_multiple_primary_throws(self):
		doc = self._make_policy(has_multiple_members=1, primary_client=None)
		self._add_member(doc, self.adult_client, is_primary=True)
		self._add_member(doc, self.other_client, is_primary=True)
		self._add_nominee(doc, self.nominee_individual)
		self._add_payment(doc)
	...
	def test_multi_member_minor_primary_throws(self):
		doc = self._make_policy(has_multiple_members=1, primary_client=None)
		self._add_member(doc, self.minor_client, is_primary=True)
		self._add_nominee(doc, self.nominee_individual)
		self._add_payment(doc)
	...
	def test_multi_member_duplicate_holder_throws(self):
		doc = self._make_policy(has_multiple_members=1, primary_client=None)
		self._add_member(doc, self.adult_client, is_primary=True)
		self._add_member(doc, self.adult_client)
		self._add_nominee(doc, self.nominee_individual)
	...
	def test_multi_member_date_of_birth_fetched(self):
		doc = self._make_policy(has_multiple_members=1, primary_client=None)
		self._add_member(doc, self.adult_client, is_primary=True)
		self._add_nominee(doc, self.nominee_individual)
		self._add_payment(doc)
	...
	def test_maturity_date_months(self):
		...
		expected = add_months(getdate("2026-06-01"), 6)
	...
	def test_maturity_date_days(self):
		...
		expected = add_days(getdate("2026-06-01"), 30)
		self.assertEqual(getdate(doc.maturity_date), expected)
	...
	def test_period_must_be_positive(self):
		doc = self._make_policy(period=0)
		self._add_nominee(doc, self.nominee_individual)
	...
	def test_period_type_required(self):
		doc = self._make_policy(period_type="")
		self._add_nominee(doc, self.nominee_individual)
	...
	def test_proposal_requires_no_policy_number(self):
		doc = self._make_policy(policy_number="POL-001")
		self._add_nominee(doc, self.nominee_individual)
	...
	def test_proposal_requires_no_policy_document(self):
		doc = self._make_policy(policy_document="/assets/test.pdf")
		self._add_nominee(doc, self.nominee_individual)
	...
	def test_proposal_succeeds_with_all_fields(self):
		doc = self._make_policy()
		self._add_nominee(doc, self.nominee_individual)
		self._add_payment(doc)
		doc.insert()
	...
	def test_renewed_succeeds(self):
		doc = self._make_policy(
			status="Renewed",
	...
	def test_surrendered_succeeds(self):
		doc = self._make_policy(
			status="Surrendered",
