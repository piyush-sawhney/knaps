...
class KNAPSBank(Document):
	...
	def _validate_second_holder(self):
		if not self.second_holder:
			return
		if self.holder_type != DOCTYPE_INDIVIDUAL:
	...
	def _populate_holder_names(self):
		if self.first_holder and self.holder_type:
			name_field = "full_name" if self.holder_type == DOCTYPE_INDIVIDUAL else "legal_name"
		...
		if self.second_holder:
			second_name = frappe.db.get_value(DOCTYPE_INDIVIDUAL, self.second_holder, "full_name")
