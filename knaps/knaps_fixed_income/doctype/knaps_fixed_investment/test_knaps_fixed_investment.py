...
class TestKNAPSFixedInvestment(IntegrationTestCase):
	...
	def _create_product_category(self, name: str) -> str:
		if frappe.db.exists("KNAPS Product Category", name):
			return name
		doc = frappe.get_doc({"doctype": "KNAPS Product Category", "category_name": name})
		doc.insert()
		return name

	def _create_product(self, name: str, category: str) -> str:
		if frappe.db.exists("KNAPS Product", name):
			return name
		doc = frappe.get_doc(
			{
	...
	def _create_non_individual_type(self, name: str) -> str:
		if frappe.db.exists("KNAPS Non Individual Type", name):
			return name
		doc = frappe.get_doc({"doctype": "KNAPS Non Individual Type", "non_individual_type": name})
		doc.insert()
		return name

	def _create_non_individual(self, legal_name: str, ni_type: str) -> str:
		existing = frappe.db.get_value("KNAPS Non Individual", {"legal_name": legal_name}, "name")
		if existing:
			return existing
		doc = frappe.get_doc(
			{
	...
	def _make_investment(self, **kwargs):
		defaults = {
			"doctype": "KNAPS Fixed Investment",
			"entry_date": today(),
			"status": "Entry Done",
